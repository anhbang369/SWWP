 ...
 * @author 84348
 */
@WebServlet(name = "SeacrhIssueController", urlPatterns = {"/SeacrhIssueController"})
public class SeacrhIssueController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = ERROR;
        try{
            String search = request.getParameter("seacrhIssue");
            DAOIssue dao = new DAOIssue();
            List<UserIssue> listUser = dao.getListIssue(search);
            if(listUser.size() >0){
                request.setAttribute("LIST_ISSUE", listUser);
                url = SUCCESS;
            }else{
